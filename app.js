...
function history(){const today=new Date().toISOString().slice(0,10);panel.innerHTML=`<h2>📚 每日開獎輸入／歷史資料</h2><p><b>每天開獎後就在這裡輸入 5 個號碼。</b> 不必做 CSV；CSV 只留給一次大量匯入。</p><div class="draw-entry"><label>開獎日期<input id="drawDate" type="date" value="${today}"></label><label>期號（可不填）<input id="drawNo" inputmode="numeric" placeholder="例如 11999"></label><div class="five-inputs">${[1,2,3,4,5].map(i=>`<input id="n${i}" inputmode="numeric" maxlength="2" placeholder="${i}">`).join('')}</div><button class="action big-action" id="saveDraw">➕ 儲存今天開獎</button><div id="drawStatus"></div></div><details class="bulk-import"><summary>大量資料才使用 CSV 匯入</summary><p>格式：date,draw,n1,n2,n3,n4,n5</p><input id="csv" type="file" accept=".csv,text/csv"><button class="action" id="clearImport">清除自行新增／匯入資料</button></details><div class="tablewrap"><table><thead><tr><th>日期</th><th>期號</th><th>號碼</th></tr></thead><tbody>${D.map(r=>`<tr><td>${r.date}</td><td>${r.draw?'#'+r.draw:'—'}</td><td>${inlineBalls(r.numbers)}</td></tr>`).join('')}</tbody></table></div>`;document.getElementById('saveDraw').onclick=saveManualDraw;document.getElementById('csv').onchange=importCSV;document.getElementById('clearImport').onclick=()=>{localStorage.removeItem('rose_ca_f5_import');location.reload()}}
function saveManualDraw(){const date=document.getElementById('drawDate').value,draw=+document.getElementById('drawNo').value||0,nums=[1,2,3,4,5].map(i=>+document.getElementById('n'+i).value).sort((a,b)=>a-b),st=document.getElementById('drawStatus');if(!date||nums.some(n=>n<1||n>39)||new Set(nums).size!==5){st.innerHTML='<div class="notice">請輸入 5 個不同的 01～39 號碼。</div>';return}let imp=JSON.parse(localStorage.getItem('rose_ca_f5_import')||'[]').filter(r=>r.date!==date);imp.push({date,draw,numbers:nums});localStorage.setItem('rose_ca_f5_import',JSON.stringify(imp));location.reload()}
